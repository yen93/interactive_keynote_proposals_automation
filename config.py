...
# Distinct from Uncharted Ice's trigger query so the same inbox can host both
# automations without either one picking up the other's emails. This default
# is a best guess at the real inbound pattern for Interactive Keynote demo-call
# notes and should be tuned once the team's actual email habits are known —
# see AS_BUILT_interactive_keynote_proposal_automation.txt "Known limitations".
GMAIL_TRIGGER_QUERY = os.environ.get(
    "GMAIL_TRIGGER_QUERY",
    'has:attachment subject:("interactive keynote" OR "keynote demo notes" OR "crossing the ice")',
)

FATHOM_API_KEY = os.environ.get("FATHOM_API_KEY", "")
FATHOM_LOOKBACK_DAYS = int(os.environ.get("FATHOM_LOOKBACK_DAYS", "30"))

SUPABASE_LOG_TABLE = "interactive_keynote_proposal_logs"
