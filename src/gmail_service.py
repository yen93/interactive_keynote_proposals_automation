...
and sending the "started" / "ready" notification messages in one thread."""
...
def search_matching_emails(gmail) -> list[dict]:
    ...
    messages that actually carry a single image/PDF attachment (the query alone
...
def start_notification_thread(gmail, subject: str, body_text: str) -> dict:
    """Sends a brand-new message (not a reply) to the notification recipients
    and returns {message_id, thread_id} for the later ready/error reply."""
...
def reply_in_thread(gmail, thread_id: str, original_message_id: str, subject: str, body_text: str) -> dict:
    """Sends a reply (ready notification or error) within the same Gmail thread."""
